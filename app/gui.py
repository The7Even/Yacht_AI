...
class YachtWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.score_labels = {}
        self.log_label = self.status_label = self.roll_label = self.turn_label = None
        self.player_total = self.ai_total = self.roll_button = None
        self.player_upper_label = self.ai_upper_label = None
        self.player_bonus_summary = self.ai_bonus_summary = None
        self.player_table_total = self.ai_table_total = None
        self.center_total_label = self.gap_label = None
        self.dice_icons = self._load_dice_icons()
        self.category_icons = self._load_category_icons()
        self._build_ui()
        self.start_new_game()

    def _load_dice_icons(self):
        icons = {}
    ...
    def _player_header(self, name, side):
        frame = QFrame()
        ...
        name_label.setStyleSheet("font-size:18px;font-weight:800;color:#64a9ff;background:transparent;" if side == "player" else "font-size:18px;font-weight:800;color:#ff7f91;background:transparent;")
        total = QLabel("0")
        total.setObjectName("score")
        total.setStyleSheet("color:#64a9ff;background:transparent;" if side == "player" else "color:#ff7f91;background:transparent;")
    ...
    def _build_center_summary(self):
        ...
        layout = QVBoxLayout(frame); layout.setContentsMargins(14, 7, 14, 7)
    ...
    def _build_scorecard(self):
        ...
        ah = QLabel("AI"); ah.setObjectName("aiHeader"); ah.setAlignment(Qt.AlignmentFlag.AlignCenter)
        header.addWidget(ph, 0, 1); header.addWidget(ah, 0, 2); layout.addLayout(header)
        layout.addWidget(self._score_section("기본 점수", UPPER_CATEGORIES))

        upper_summary = QFrame(); upper_summary.setObjectName("scoreRow")
        summary = QGridLayout(upper_summary); summary.setContentsMargins(5, 4, 5, 4); summary.setHorizontalSpacing(4)
        summary.addWidget(QLabel("상단 합계"), 0, 0)
        self.player_upper_label = QLabel("0 / 63"); self.player_upper_label.setObjectName("playerCell"); self.player_upper_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.ai_upper_label = QLabel("0 / 63"); self.ai_upper_label.setObjectName("aiCell"); self.ai_upper_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        summary.addWidget(self.player_upper_label, 0, 1); summary.addWidget(self.ai_upper_label, 0, 2)
        summary.addWidget(QLabel("보너스"), 1, 0)
        self.player_bonus_summary = QLabel("-"); self.player_bonus_summary.setObjectName("playerCell"); self.player_bonus_summary.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.ai_bonus_summary = QLabel("-"); self.ai_bonus_summary.setObjectName("aiCell"); self.ai_bonus_summary.setAlignment(Qt.AlignmentFlag.AlignCenter)
        summary.addWidget(self.player_bonus_summary, 1, 1); summary.addWidget(self.ai_bonus_summary, 1, 2); layout.addWidget(upper_summary)
        layout.addWidget(self._score_section("특수 족보", SPECIAL_CATEGORIES))

        total_row = QFrame(); total_row.setObjectName("scoreRow")
        total = QGridLayout(total_row); total.setContentsMargins(5, 4, 5, 4); total.setHorizontalSpacing(4); total.addWidget(QLabel("총합"), 0, 0)
        self.player_table_total = QLabel("0"); self.player_table_total.setObjectName("playerCell"); self.player_table_total.setAlignment(Qt.AlignmentFlag.AlignCenter)
        self.ai_table_total = QLabel("0"); self.ai_table_total.setObjectName("aiCell"); self.ai_table_total.setAlignment(Qt.AlignmentFlag.AlignCenter)
        total.addWidget(self.player_table_total, 0, 1); total.addWidget(self.ai_table_total, 0, 2); layout.addWidget(total_row)
        return card

    def _score_section(self, title, categories):
        wrap = QFrame(); layout = QVBoxLayout(wrap); layout.setContentsMargins(0, 0, 0, 0); layout.setSpacing(2)
        subsection = QLabel(title); subsection.setObjectName("subsection"); layout.addWidget(subsection)
        grid = QGridLayout(); grid.setContentsMargins(0, 0, 0, 0); grid.setHorizontalSpacing(4); grid.setVerticalSpacing(2)
        for row, category in enumerate(categories):
            name = QLabel(CATEGORY_SHORT[category]); name.setStyleSheet("font-size:12px;"); grid.addWidget(name, row, 0)
            for column, player in ((1, PlayerId.PLAYER), (2, PlayerId.AI)):
                value = QLabel("-"); value.setObjectName("playerCell" if player is PlayerId.PLAYER else "aiCell"); value.setAlignment(Qt.AlignmentFlag.AlignCenter)
                self.score_labels[player, category] = value; grid.addWidget(value, row, column)
        layout.addLayout(grid); return wrap

    def _build_play_area(self):
        card = QFrame(); card.setObjectName("card")
        layout = QVBoxLayout(card); layout.setContentsMargins(14, 9, 14, 9); layout.setSpacing(6)
        top = QHBoxLayout(); caption = QLabel("주사위 굴리기"); caption.setObjectName("section"); top.addWidget(caption); top.addStretch()
        self.roll_label = QLabel("3회 남음"); self.roll_label.setObjectName("rollRemaining"); top.addWidget(self.roll_label); layout.addLayout(top)
        ...
        title = QLabel("카테고리 선택"); title.setObjectName("section"); layout.addWidget(title)
        layout.addWidget(self._category_group("기본 점수 (Ones ~ Sixes)", UPPER_CATEGORIES)); layout.addWidget(self._category_group("특수 족보", SPECIAL_CATEGORIES, special=True))
        return card
    ...
    def _category_group(self, title, categories, special=False):
        ...
        for index, category in enumerate(categories):
            ...
            if category in self.category_icons:
                button.setIcon(self.category_icons[category]); button.setIconSize(QSize(88, 42))
            elif category.is_upper and category.upper_face in self.dice_icons:
                button.setIcon(self.dice_icons[category.upper_face]); button.setIconSize(QSize(34, 34))
            button.setText(f"{CATEGORY_SHORT[category]}\n사용 가능 점수: -")
            button.clicked.connect(lambda _=False, cat=category: self.score_category(cat)); self.category_buttons[category] = button; grid.addWidget(button, index // 3, index % 3)
        layout.addLayout(grid); return wrap

    def _build_ai_panel(self):
        outer = QFrame(); outer.setObjectName("card")
        layout = QVBoxLayout(outer); layout.setContentsMargins(9, 9, 9, 9)
        title = QLabel("AI 진행 상황"); title.setObjectName("section"); layout.addWidget(title)
        ai_status = QLabel("AI (FastEV)\n\n게임 엔진 연결 준비가 완료되었습니다.\n\n현재는 PLAYER 프로토타입 단계입니다."); ai_status.setAlignment(Qt.AlignmentFlag.AlignCenter); ai_status.setObjectName("status"); layout.addWidget(ai_status, 1)
        log_title = QLabel("게임 로그"); log_title.setObjectName("section"); layout.addWidget(log_title)
        self.log_label = QLabel("게임이 시작되었습니다.\nPLAYER 턴입니다."); self.log_label.setObjectName("log"); self.log_label.setWordWrap(True); layout.addWidget(self.log_label)
        return outer

    def start_new_game(self):
        self.engine.start_game(); self._refresh(); self._set_status("0/3회 굴렸습니다. 주사위를 클릭하면 HOLD할 수 있습니다.")

    def roll_dice(self):
        try: dice = self.engine.roll_dice()
        except (RuntimeError, ValueError) as exc: self._set_status(str(exc)); return
        self._refresh(dice); self._set_status(f"{self.engine.state.roll_count}/{MAX_ROLLS_PER_TURN}회 굴렸습니다. 주사위를 클릭하면 HOLD할 수 있습니다.")

    def toggle_hold(self, index):
        if self.engine.state.current_dice is None: return
        try:
    ...
    def score_category(self, category):
        ...
        if self.engine.is_game_over():
            self._refresh(); QMessageBox.information(self, "게임 종료", f"게임이 종료되었습니다.\nPLAYER {self.engine.state.player_score}점"); return
        self._set_status(f"{CATEGORY_SHORT[category]}에 {score}점을 기록했습니다. AI 턴은 다음 단계에서 연결합니다."); self.engine.end_turn(); self._refresh()

    def _refresh(self, dice: Iterable[int] | None = None):
        state = self.engine.state; dice = tuple(dice) if dice is not None else state.current_dice
        for index, button in enumerate(self.die_buttons):
            ...
            button.setProperty("held", index in state.held_indices); button.style().unpolish(button); button.style().polish(button)
            button.setEnabled(value is not None and state.current_player is PlayerId.PLAYER and not state.turn_scored)
        remaining = max(0, MAX_ROLLS_PER_TURN - state.roll_count); self.roll_label.setText(f"{remaining}회 남음")
        ...
        available = set(self.engine.get_available_categories()) if state.current_dice else set(); scores = self.engine.get_current_scores() if state.current_dice else {}
        for category, button in self.category_buttons.items():
            is_available = category in available; button.setProperty("available", is_available)
            button.setText(f"{CATEGORY_SHORT[category]}\n사용 가능 점수: {scores[category]}점" if is_available else f"{CATEGORY_SHORT[category]}\n사용됨"); button.setEnabled(is_available); button.style().unpolish(button); button.style().polish(button)
        ...
        if diff > 0: gap_text, gap_object = f"점수 차이 {diff}점 · PLAYER 우세", "scoreGap"
        elif diff < 0: gap_text, gap_object = f"점수 차이 {abs(diff)}점 · AI 우세", "scoreGapNegative"
        else: gap_text, gap_object = "점수 차이 0점 · 동점", "scoreGap"
        ...
        self.player_upper_label.setText(f"{state.player_upper_total} / 63"); self.ai_upper_label.setText(f"{state.ai_upper_total} / 63")
        self.player_bonus_summary.setText("획득 (+35)" if state.player_has_bonus else "-"); self.ai_bonus_summary.setText("획득 (+35)" if state.ai_has_bonus else "-")
        self.player_table_total.setText(str(state.player_score)); self.ai_table_total.setText(str(state.ai_score)); round_no = min(12, (state.turn + 1) // 2); self.turn_label.setText(f"턴 {round_no} / 12")
